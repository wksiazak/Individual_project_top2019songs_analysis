...
#możemy to zrobić jeszcze raz, ale tylko na wybranych danych , tzn znaczących: dance pop, canadian pop, canadian contemporary r&b, pop


# Subset only the hip-hop tracks, and then only the rock tracks
dancePop_only = spotify_data_2010_2019.loc[spotify_data_2010_2019["top genre"] == "dance pop"]

# The features, labels, and pca projection are created for the balanced dataframe
features_danePop = dancePop_only.drop(['title', 'artist','top genre', 'number'], axis=1)
labels_dancePop = dancePop_only['top genre']

pca_projection_dancePop = pca.fit_transform(scaler.fit_transform(features_danePop))

# Redefine the train and test set with the pca_projection from the balanced data
train_features, test_features, train_labels, test_labels = train_test_split(pca_projection_dancePop, labels_dancePop, random_state=10)

# Train our decision tree on the balanced data
tree_dancePop = DecisionTreeClassifier(random_state=10)
tree_dancePop.fit(train_features, train_labels)
pred_labels_tree_dancePop = tree_dancePop.predict(test_features)

# Train our logistic regression on the balanced data
logreg_dancePop = LogisticRegression(random_state=10)
logreg_dancePop.fit(train_features, train_labels)
pred_labels_logit_dancePop = logreg.predict(test_features)

# Compare the models
print("Decision Tree only for dance pop : \n", classification_report(test_labels, pred_labels_tree_dancePop))
"""